...
std::pair<std::string, std::string> splitParams(const std::string& param) {
    ...
    if (divider_index == std::string::npos) {
        throw std::invalid_argument("[Config] Wrong config parameter recieved. Take a look in README.md.");
    }
...
void readConfig (Config& config) {
    ...
    std::string param;
    std::cout << "[Config] Config initiated successfully.\n";
    while (std::getline(input_stream, param)) {
        ...
        if (splited_params.first == "path") {
            ...
            throw std::invalid_argument("[Config] Wrong cfg parameter recieved: " + splited_params.first);
        }
    }
    std::cout << "[Config] Config read successfully.\n";
}